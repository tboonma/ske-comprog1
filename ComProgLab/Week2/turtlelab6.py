...
class Task:
    """
    TASK: help guide Turtle back home while stop to visit Mozart and
    Raph Mozart's, Raph's, and Turtle's home will randomly appear
    ANYWHERE on the map.
    """
    def start(self):
        stage.reset()
        ...
        if INTERACTIVE:
            ...
            shuffle(locations)
            (mozart_x,mozart_y),(raph_x,raph_y) = locations
        else:
            mozart_x,mozart_y = int(input()),int(input())
            raph_x,raph_y = int(input()),int(input())
            home_x,home_y = int(input()),int(input())

        self.home = Home(home_x,home_y,32,32,IMG_HOME,"home")
        self.mozart = Home(mozart_x,mozart_y,32,32,IMG_HOME,"mozart")
        self.raph = Home(raph_x,raph_y,32,32,IMG_HOME,"raph")
        stage.add_object(self.home)
        stage.add_object(self.mozart)
        stage.add_object(self.raph)
        stage.add_object(Road(0,0,mozart_x,mozart_y,5,"yellow"))
        stage.add_object(Road(0,0,raph_x,raph_y,5,"yellow"))
        stage.add_object(Road(mozart_x,mozart_y,raph_x,raph_y,5,"yellow"))
        stage.add_object(Road(home_x,home_y,mozart_x,mozart_y,5,"yellow"))
        stage.add_object(Road(home_x,home_y,raph_x,raph_y,5,"yellow"))
        if INTERACTIVE:
            print(dedent(self.__doc__).strip())
            print(f"Mozart appears at ({self.mozart.x:.2f},{self.mozart.y:.2f})")
            print(f"Raph appears at ({self.raph.x:.2f},{self.raph.y:.2f})")
            print(f"Home appears at ({self.home.x:.2f},{self.home.y:.2f})")
        stage.total_distance = 0
        return stage.turtle,self.home,self.mozart,self.raph

    def shortest_path(self):
        dist1 = ( distance(0,0,self.mozart.x,self.mozart.y) +
                  distance(self.mozart.x,self.mozart.y,self.raph.x,self.raph.y) +
                  distance(self.raph.x,self.raph.y,self.home.x,self.home.y) )
        dist2 = ( distance(0,0,self.raph.x,self.raph.y) +
                  distance(self.raph.x,self.raph.y,self.mozart.x,self.mozart.y) +
                  distance(self.mozart.x,self.mozart.y,self.home.x,self.home.y) )
    ...
    def done(self):
        ...
        if INTERACTIVE:
            ...
            print("Seed =",4)
        print("Check if Turtle stopped at Mozart's: ", end="")
        if any(self.mozart.contains(stop) for stop in stage.stops):
            print("PASSED")
        else:
            print("FAILED")
        print("Check if Turtle stopped at Raph's: ", end="")
        if any(self.raph.contains(stop) for stop in stage.stops):
            print("PASSED")
        else:
            print("FAILED")
...
task = Task()
turtle,home,mozart,raph = task.start()
mod.check = task.done
mod.turtle = turtle
mod.home = home
mod.mozart = mozart
