...
class Task:
    """
    TASK: Help Turtle to visit his friend, Mozart.
    ...
    def start(self):
        ...
        if INTERACTIVE:
            ...
            angle = randint(0,360)
            mozart_x = cos(radians(angle))*distance
            mozart_y = sin(radians(angle))*distance
        else:
            mozart_x = int(input("Enter Mozart's x: "))
            mozart_y = int(input("Enter Mozart's y: "))
        self.mozart = Home(mozart_x,mozart_y,32,32,IMG_HOME,"Mozart")
        stage.add_object(self.mozart)
        if INTERACTIVE:
            print(dedent(self.__doc__).strip())
            print(f"Mozart's home is at ({self.mozart.x:.2f},{self.mozart.y:.2f})")
    ...
    def done(self):
        ...
        if INTERACTIVE:
            ...
            print("Seed =",11)
        print("Check if Turtle visited Mozart: ", end="")
        print(visited(self.mozart))
